...
- 2026-09-23: Stop alarms model weekly recurrence as `repeatWeekdays: number[]` masks (0=Sunday..6, empty = one-shot; replace legacy `repeatDaily` semantics anywhere). Roll-forward helpers (`computeNextTriggerAt`, `advanceRepeatTrigger`) land on the next selected weekday; persist masks sorted/deduped through `normalizeRepeatWeekdays`; the creation dialog bypasses the lead-time guard only while recurrence is on. Pinned departure bubble: the bubble itself opens the search, dismiss uses a trash icon, countdown shows a clock icon without a textual prefix.
- 2026-09-24: `src/assets/runtime-flags.js` ships with the production default `{ forceSnapshot: false, mockDataMode: null }` — live CTAN API first, packaged snapshot only as fallback when the API is unavailable (`StopScheduleService` live→snapshot `catchError`). The dev-mode rewrites from `scripts/dev/start-with-*.mjs` are transient: `scripts/runtime-flags.test.ts` fails `test:scripts` if a committed file enables a mock mode, forces snapshots, or writes to `localStorage` (a committed mock state previously reset saved preferences and silently stopped persistence on every page load).
- 2026-09-24: Alarms are editable from the alarms page: non-expired rows expose an edit action that reuses `StopAlarmDialogComponent` in edit mode (prefilled offset via matching chip or custom value, prefilled weekday mask, title `stopDetail.alarms.editTitle`, no "arrives in" suffix). Edits persist through `StopAlarmsService.update`, which preserves identity and enabled state and drops the alarm when the new schedule can never ring again.
- 2026-09-22: Live trip tracking lives under `src/app/domain/trip/` (`trip-progress.util.ts` polyline projections, `live-trip.service.ts` GPS watch session, `trip-session.storage.ts` persistence) plus the `src/app/features/trip/` full-screen timeline on `APP_CONFIG.routes.trip`. Pinned departure tracking lives in `PinnedDepartureService`/`PinnedDepartureStorage` with the shell-level `PinnedDepartureIndicatorComponent`; both features keep translations under `trip.*`, `layout.pinnedDeparture.*`, and `routeSearch.pin*` in the locale JSONs.
- 2026-09-26: Live trip ETAs are GPS-anchored: `LiveTripService` re-anchors the plan pace to the latest fix (`estimateEtaMs`, 1s tick, `planSpanMs`/`progressAt` in state), marks arrival when a fix lands within `TRIP_ARRIVAL_RADIUS_METERS` of the destination, and the trip view shows minimal-unit countdowns (s/m/h) instead of timetable clocks, treating a zero countdown as arrival. The trip timeline groups consecutive stops per nucleus with per-nucleus ordinals, every stop row/marker opens an info popover (nucleus, stop number, live ETA, stop-detail link), the recenter GPS button lives inside the sticky destination block, and a list/map toggle renders the route live on `RouteMapComponent` (new `userPosition` input + `centerOnUser()`), auto-following the next stop with a dismissible popover.
- 2026-09-26: Ride detection accepts timetable rows up to 30 minutes in the past (late buses) and stays silent when an open schedule view already covers a candidate: pages register `RideOpenScheduleContext`s (`registerOpenSchedule`/`unregisterOpenSchedule` from stop detail and route search), `visibleProposals` filters the floating bubble, and the bubble also hides while a live trip session is active.
- 2026-09-26: Live trip countdowns are one consistent GPS-anchored number everywhere: the sticky destination ETA reads the destination stop view the timeline renders (never a separate service-side estimate), and the next-stop announcement quotes that stop's own countdown. Transient `watchPosition` errors no longer wipe the last fix (`LiveTripService.handleError` keeps progress/anchor so the timeline never resets and nothing falls back to the timetable alone); once no fresh fix arrives for `TRIP_GPS_STALE_MS` (60s), the anchor expires and every countdown degrades to the timetable together (`trip.unavailable` wording reflects the kept last known location). The live trip map follows the user: `RouteMapComponent` gains a `followUser` input (first fix flies in at `USER_FOCUS_ZOOM`, later fixes pan via new `MapHandle.panTo`), suspends on user drag or stop-popup inspection (`MapHandle.onUserPanStarted`, Leaflet `dragstart`), and the sticky recenter action resumes following.
- 2026-09-22: Map re-centers smoothly on stop selection: `MapHandle.centerStop(stopId, animate?)` (leaflet-map.service) pans with the shared camera duration; workspace panels and marker clicks call it before raising selection. Note `@ViewChild` must query template references (e.g. `'routeMap'`) rather than component classes so `TestBed.overrideComponent` stubs still match in specs. Recent-search preview rows are fixed-column grids (badge / wait / right-aligned tabular time) with no direction arrows: past rows use muted inverse text and the upcoming row uses primary highlight tokens exposed from the card host; keep spoken content in a visually-hidden node and mark the visual columns `aria-hidden`.
