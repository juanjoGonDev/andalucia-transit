...
const RECORD_LOG_PREFIX = '[record]';
...
interface RecordEventPayload {
  [EVENT_KEY]?: string;
  [DETAILS_KEY]?: Record<string, unknown>;
}
...
function buildRecordArgs(config: RecordConfig): string[] {
  const args = normalizePassThrough(config.passThrough);
...
function normalizePassThrough(values: string[]): string[] {
  const normalized: string[] = [];
  for (let index = 0; index < values.length; index += 1) {
    const current = values[index];
    const next = values[index + 1];
    if (current.startsWith(FLAG_PREFIX) && !current.includes(FLAG_VALUE_SEPARATOR) && next && !next.startsWith(FLAG_PREFIX)) {
      normalized.push(`${current}${FLAG_VALUE_SEPARATOR}${next}`);
      index += 1;
      continue;
    }
...
function processStdout(buffer: string, collector: string[]): string {
  ...
  while (true) {
    ...
    try {
      const payload = JSON.parse(line) as RecordEventPayload;
      logRecordEvent(payload);
      const details = payload[DETAILS_KEY];
      if (payload[EVENT_KEY] === SCREENSHOT_EVENT && details) {
        const screenshotPath = details[PATH_KEY];
        if (typeof screenshotPath === 'string') {
          collector.push(screenshotPath);
        }
      }
    } catch {
      continue;
...
function logRecordEvent(payload: RecordEventPayload): void {
  const eventName = payload[EVENT_KEY];
  if (!eventName) {
    return;
  }
  const detailValue = payload[DETAILS_KEY];
  const serializedDetails = detailValue && Object.keys(detailValue).length > 0 ? ` ${JSON.stringify(detailValue)}` : '';
  process.stderr.write(`${RECORD_LOG_PREFIX} ${eventName}${serializedDetails}\n`);
}
