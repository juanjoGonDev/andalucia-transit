...
const DEFAULT_MEDIA_OUTPUT_DIR = 'examples/media';
const DEFAULT_NAME = 'capture';
const DEFAULT_VIDEO_FILE_NAME = 'recording.mp4';
const MILLISECONDS_IN_SECOND = 1000;
const VIDEO_TEMP_PREFIX = 'record-video-';
const HELP_FLAG = 'help';
const FILE_NAME_FLAG = 'fileName';
const DURATION_FLAG = 'duration';
const MARGIN_START_FLAG = 'marginStart';
const MARGIN_END_FLAG = 'marginEnd';
const OUTPUT_DIR_FLAG = 'outputDir';
const SCREENSHOT_NAME_FLAG = 'screenshotName';
const IMAGE_EXTENSION_PNG = '.png';
const IMAGE_EXTENSION_JPEG = '.jpeg';
const IMAGE_EXTENSION_JPG = '.jpg';
const VIDEO_EXTENSION_MP4 = '.mp4';
const VIDEO_EXTENSION_WEBM = '.webm';
const RUN_MODE_RECORD = 'record';
const RUN_MODE_SCREENSHOT = 'screenshot';
...
const SCREENSHOT_FORMATS = new Map([
  [IMAGE_EXTENSION_PNG, { type: 'png' }],
  [IMAGE_EXTENSION_JPEG, { type: 'jpeg' }],
  [IMAGE_EXTENSION_JPG, { type: 'jpeg' }],
]);
const VIDEO_FORMATS = new Map([
  [VIDEO_EXTENSION_MP4, { container: 'mp4' }],
  [VIDEO_EXTENSION_WEBM, { container: 'webm' }],
]);
const HELP_TEXT = [
  'Usage: node scripts/record.js [flags]',
  '',
  'Core flags:',
  '  --url=<target> (required)',
  '  --name=<base screenshot name>',
  '  --outDir=<screenshot directory>',
  '  --fullPage=true|false',
  '  --headless=true|false',
  '  --locale=es,en',
  '',
  ...
  '  --marginStart=<seconds>',
  '  --marginEnd=<seconds>',
  '  --outputDir=<media directory>',
  '  --screenshotName=<image file name>',
  '',
...
function resolveMediaTarget(directory, candidateName, defaultName, defaultExtension, formats, label) {
  const trimmed = candidateName ? String(candidateName).trim() : '';
  const name = trimmed.length > 0 ? trimmed : defaultName;
  const extension = normalizeExtension(name);
  const effectiveExtension = extension.length > 0 ? extension : defaultExtension;
  const format = formats.get(effectiveExtension);
  if (!format) {
    throw new Error(`${label} extension not supported`);
  }
  const finalName = extension.length > 0 ? name : `${name}${defaultExtension}`;
  return { path: path.join(directory, finalName), format, extension: effectiveExtension };
}

function toMilliseconds(seconds) {
  return Math.max(0, Math.round(seconds * MILLISECONDS_IN_SECOND));
}
...
async function persistVideo(video, targetPath) {
  if (!video) {
    return;
  }
 ...
 * @param {string[]} argv
 * @param {string} mode
 * @param {{ chromium?: import('playwright').ChromiumBrowserType }} dependencies
 */
async function executeCli(argv, mode, dependencies = { chromium }) {
  ...
  const helpRequested = readBoolean(options[HELP_FLAG], false);
  if (helpRequested) {
  ...
  const headless = readBoolean(options.headless, true);
  ...
  const screenshotOutputDir = path.resolve(readSingle(options.outDir) ?? DEFAULT_OUTPUT_DIR);
  const mediaOutputDir = path.resolve(readSingle(options[OUTPUT_DIR_FLAG]) ?? DEFAULT_MEDIA_OUTPUT_DIR);
  ...
  const harPath = harRaw ? (path.isAbsolute(harRaw) ? harRaw : path.join(screenshotOutputDir, harRaw)) : null;
  const consolePath = consoleRaw ? (path.isAbsolute(consoleRaw) ? consoleRaw : path.join(screenshotOutputDir, consoleRaw)) : null;
  ...
  const durationSeconds = readNumber(options[DURATION_FLAG], 0);
  const marginStartSeconds = readNumber(options[MARGIN_START_FLAG], 0);
  const marginEndSeconds = readNumber(options[MARGIN_END_FLAG], 0);
  if (durationSeconds < 0 || marginStartSeconds < 0 || marginEndSeconds < 0) {
    throw new Error('Recording durations must be zero or positive');
  }
  const videoDurationMs = toMilliseconds(durationSeconds);
  const marginStartMs = toMilliseconds(marginStartSeconds);
  const marginEndMs = toMilliseconds(marginEndSeconds);
  const videoNameCandidate = readSingle(options[FILE_NAME_FLAG]);
  const screenshotOverrideName = readSingle(options[SCREENSHOT_NAME_FLAG]);
  const shouldRecordVideo = Boolean(videoNameCandidate) || videoDurationMs > 0 || marginStartMs > 0 || marginEndMs > 0;
  if (shouldRecordVideo && videoDurationMs === 0) {
    throw new Error('Video duration must be greater than zero');
  }
  const screenshotDirectory = screenshotOverrideName ? mediaOutputDir : screenshotOutputDir;
  ensureDirectory(screenshotOutputDir);
  if (shouldRecordVideo || screenshotOverrideName) {
    ensureDirectory(mediaOutputDir);
  }
  const startPayload = { url: targetUrl, outputDir: screenshotOutputDir };
  if (shouldRecordVideo || screenshotOverrideName) {
    startPayload.mediaDir = mediaOutputDir;
  }
  logEvent(LOG_EVENT_START, startPayload);
  const videoTarget = shouldRecordVideo
    ? resolveMediaTarget(mediaOutputDir, videoNameCandidate, DEFAULT_VIDEO_FILE_NAME, VIDEO_EXTENSION_MP4, VIDEO_FORMATS, 'Video')
    : null;
  const chromiumApi = dependencies && dependencies.chromium ? dependencies.chromium : chromium;
  const browser = await chromiumApi.launch({ headless });
  try {
    const breakpoints = breakpointValues.length > 0 ? breakpointValues : [{ width, height }];
    if (shouldRecordVideo && breakpoints.length !== 1) {
      throw new Error('Video capture requires a single breakpoint');
    }
    if (shouldRecordVideo && locales.length !== 1) {
      throw new Error('Video capture requires a single locale');
    }
      ...
      for (const breakpoint of breakpoints) {
        await withRetries(async () => {
          ...
          if (shouldRecordVideo) {
            tempVideoDir = fs.mkdtempSync(path.join(os.tmpdir(), VIDEO_TEMP_PREFIX));
            contextOptions.recordVideo = { dir: tempVideoDir, size: viewport };
          }
          const context = await browser.newContext(contextOptions);
          let videoArtifact = null;
          let actionStartTimestamp = 0;
          try {
            if (offline) {
            ...
            logEvent(LOG_EVENT_BROWSER, { viewport, locale, userAgent, colorScheme, ignoreHttpsErrors, headless });
            ...
            if (shouldRecordVideo && marginStartMs > 0) {
              await page.waitForTimeout(marginStartMs);
            }
            if (shouldRecordVideo) {
            ...
            const baseName = captureName(nameBase, viewport.width, viewport.height, locale, variant);
            const screenshotTarget = screenshotOverrideName
              ? resolveMediaTarget(screenshotDirectory, screenshotOverrideName, baseName, IMAGE_EXTENSION_PNG, SCREENSHOT_FORMATS, 'Screenshot')
              : { path: path.join(screenshotOutputDir, `${baseName}${IMAGE_EXTENSION_PNG}`), format: SCREENSHOT_FORMATS.get(IMAGE_EXTENSION_PNG), extension: IMAGE_EXTENSION_PNG };
            ensureDirectory(path.dirname(screenshotTarget.path));
            if (elementSelector) {
              const elementHandle = await page.$(elementSelector);
              if (!elementHandle) {
                throw new Error('Target element not found');
              }
              await elementHandle.screenshot({ path: screenshotTarget.path, type: screenshotTarget.format?.type });
            } else {
              const screenshotOptions = { path: screenshotTarget.path, fullPage };
              if (clipRegion) {
                screenshotOptions.clip = clipRegion;
              }
              if (screenshotTarget.format?.type) {
                screenshotOptions.type = screenshotTarget.format.type;
              }
              await page.screenshot(screenshotOptions);
            }
            logEvent(LOG_EVENT_SCREENSHOT, { path: screenshotTarget.path });
            if (shouldRecordVideo) {
              const actionElapsed = Date.now() - actionStartTimestamp;
              const remaining = Math.max(videoDurationMs - actionElapsed, 0);
              if (remaining > 0) {
                await page.waitForTimeout(remaining);
              }
              if (marginEndMs > 0) {
                await page.waitForTimeout(marginEndMs);
              }
            }
            if (storageStatePath) {
            ...
            await context.close();
            if (shouldRecordVideo && videoArtifact) {
              await persistVideo(videoArtifact, videoTarget.path);
              logEvent(LOG_EVENT_VIDEO, { path: videoTarget.path, format: videoTarget.extension.slice(1) });
            }
            if (shouldRecordVideo && tempVideoDir) {
...
async function runCli(argv, mode, dependencies) {
  try {
    const exitCode = await executeCli(argv, mode, dependencies);
...
function runRecordCli(argv, dependencies) {
  return runCli(argv, RUN_MODE_RECORD, dependencies);
}

function runScreenshotCli(argv, dependencies) {
  return runCli(argv, RUN_MODE_SCREENSHOT, dependencies);
}
