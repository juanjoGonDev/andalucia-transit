...
- **Implementation Notes:** Use `appAccessibleButton` with `[appAccessibleButtonTabIndex]`, handle directional keys with `matchesKey` helpers, call `queueMicrotask`-driven focus after updating signals, persist selection via query param canonicalization plus storage, and restore focus after navigation events.
