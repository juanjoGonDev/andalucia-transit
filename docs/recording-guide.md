...
- Upload the resulting screenshot and video to https://filebin.net/ by running `npm run publish:evidence -- --url <pageUrl> --label "<Surface name>"` (append additional recorder flags after a `--` separator when scenarios require interaction scripts), capture the markdown block it prints, add the Filebin links to the pull request summary or comments, and delete the local binaries afterward.
