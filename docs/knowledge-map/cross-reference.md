...
| Theme tokens & contrast | `src/styles/theme-rules.spec.ts`, `src/theme/tokens.ts` | [`ui-theme`](../ui-theme.md), [`contrast log`](../audit/contrast-report.md) | Playwright `tests/playwright/theme.contrast.spec.ts` | Theme tokens in CSS variables |
