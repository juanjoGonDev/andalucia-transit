...
test.describe('Legal and privacy surfaces', () => {
  ...
  test('shows an informational first-visit notice and persists dismissal', async ({ page }) => {
    ...
    await assertControlIsUnobscured(
      page,
      notice.getByRole('button', { name: 'Entendido' }),
  ...
  test('keeps the legal footer visually continuous with hero and plain routed surfaces', async ({
    page
  }) => {
    ...
    for (const viewport of [MOBILE_VIEWPORT, DESKTOP_VIEWPORT]) {
      ...
      await open(page, '/');
      await assertFooterSurface(page, 'hero', '--color-secondary');
      await assertNoHorizontalOverflow(page);

      await open(page, '/legal/privacy');
      await assertFooterSurface(page, 'plain', '--color-surface-muted');
      await assertNoHorizontalOverflow(page);

      const privacyLink = page.getByRole('link', { name: 'Privacidad' }).last();
      await privacyLink.scrollIntoViewIfNeeded();
      await assertControlIsUnobscured(page, privacyLink, viewport.height);
    }
  });
...
async function assertFooterSurface(
  page: Page,
  surface: 'hero' | 'plain',
  expectedBackgroundToken: string
): Promise<void> {
  const routedSurface = page.locator(`.app-layout__surface--${surface}`);
  const footer = page.locator('.legal-footer');

  await expect(routedSurface).toBeVisible();
  await expect(footer).toBeVisible();
  await expect(footer).toHaveClass(new RegExp(`legal-footer--${surface}`, 'u'));

  const routedBox = await routedSurface.boundingBox();
  const footerBox = await footer.boundingBox();
  expect(routedBox).not.toBeNull();
  expect(footerBox).not.toBeNull();

  if (routedBox && footerBox) {
    expect(Math.abs(routedBox.y + routedBox.height - footerBox.y)).toBeLessThanOrEqual(1);
  }

  const [footerBackground, expectedBackground] = await Promise.all([
    footer.evaluate((element) => getComputedStyle(element).backgroundColor),
    resolveColorToken(page, expectedBackgroundToken)
  ]);
  expect(footerBackground).toBe(expectedBackground);
}
...
async function assertControlIsUnobscured(
  page: Page,
  control: Locator,
